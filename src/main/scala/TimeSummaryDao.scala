...
class TimeSummaryDao {
  def insertTimeSummary(summaryItems:ArrayBuffer[(Int, Int, Int, Int)]) = {
    ...
    try {
      ...
      summaryItems.foreach { item =>
        statement.executeUpdate("insert into time_summary(bus_stop_id, route_id, type, position) values('" + item._1 + "', '" + item._2 +"', '" + item._3 +"', '" + item._4 + "')")
